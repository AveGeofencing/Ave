...
#                 jti = UUID(
#                     jti
#                 )  # make sure the jti is UUID type (as is in the databse) before using it for operations
...
class PasswordResetToken(BaseToken):
    ...
    # noinspection PyMethodOverriding
    @classmethod
    async def new(cls, user_id: str) -> str:
        return await super().new(sub=user_id)

    @classmethod
    async def decode(cls, token: str) -> str:
        claims = await super().decode(token)
        user_id = claims["sub"]
        return user_id
